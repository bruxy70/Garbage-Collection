...
|`period` | Yes | If `period` is not defined (or equal to 1), the schedule will repeat every month. If `period` is 2, it will be every 2<sup>nd</sup>month, and so on. The `first_month` parameter will then define the starting month. So if the `first_month` is `jan` (or not defined), and period is 2, the collection will be in jan, mar, may, jul, sep, nov - odd months. If `first_month` is `feb`, it will be on even months. If `period` is 3, it will be once per quarter.<br/>(integer 1-12)<br/>**Default**: 1
